...
После этого открой `Logs` у сервиса. При успешном подключении в строке запуска будет указано `PostgreSQL + JSON backup`. Публичный `/api/health` специально не показывает настройки и внутренние пути сервера.

Если в логах указано только `JSON`, приложение всё равно запустится, но аккаунты, сообщения и файлы могут пропасть после перезапуска. Для настоящего сайта исправь `DATABASE_URL`.

## 2.3. Настроить защищённый вход администратора

В `Environment` добавь две переменные:

```text
ADMIN_LOGIN_EMAIL=твоя-почта@example.com
ADMIN_LOGIN_PASSWORD=длинный-уникальный-пароль-не-менее-16-символов
```

Не публикуй этот пароль в GitHub и не используй его на других сайтах. После изменения пароля все старые админские сеансы автоматически отзываются.
