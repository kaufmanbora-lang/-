...
const NFT_BACKGROUNDS = [...NFT_PHOTO_BACKGROUNDS, ...NFT_LEGACY_BACKGROUNDS];
...
function nftMarketData(viewer) {
  const featured = nftVisibleBackgrounds().flatMap((background) => (
    nftVisibleSymbols().map((symbol) => serializeNftCombo(background, symbol))
  ))
...
async function handleApi(req, res, url) {
  ...
  if (url.pathname === "/api/nft/buy" && req.method === "POST") {
    ...
    if (background.legacy || !symbol.image) return sendError(res, 400, "This NFT is no longer available in the primary market.");
