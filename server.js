...
const NFT_BACKGROUNDS = [...NFT_PHOTO_BACKGROUNDS, NFT_LOKOCHKA_BACKGROUND, ...NFT_LEGACY_BACKGROUNDS];
...
function nftVariantImage(background, symbol) {
  ...
  return symbol.variants?.[background.id] || symbol.image || background.image || "";
}
...
function nftPrimaryPrice(backgroundId, symbolId) {
  ...
  if (symbol.id === "lokochka") {
    ...
    const growth = Math.min(55000 - basePrice, sold * 650);
    return Math.min(55000, Math.max(33000, Math.round((basePrice + growth) / 100) * 100));
  }
