...
function nftPrimaryPrice(backgroundId, symbolId) {
  ...
  if (symbol.id === "lokochka") {
    const rarityPrice = 32000 + background.tier * 2400 + symbol.tier * 4200 + background.tier * symbol.tier * 210;
    const popularityPrice = sold * Math.max(2200, Math.round((background.tier + symbol.tier) * 260));
    return Math.max(120000, Math.round((rarityPrice + popularityPrice) / 100) * 100);
  }
