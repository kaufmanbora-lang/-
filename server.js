...
async function handleApi(req, res, url) {
  ...
  if (url.pathname === "/api/stories" && req.method === "POST") {
    ...
    if (body.attachment?.data) {
      attachment = saveUpload(body.attachment, { maxBytes: MAX_AVATAR_BYTES, imagesOnly: true });
    }
    if (!text && !attachment) return sendError(res, 400, "Добавь текст или фото для истории.");
    const story = {
      id: uid("story"),
      userId: auth.user.id,
      text,
  ...
  if (url.pathname === "/api/minigames/play" && req.method === "POST") {
    ...
    const body = await readBody(req);
    const game = ["dice", "coin", "spin", "reactor", "memory", "signal"].includes(String(body.game || "")) ? String(body.game) : "dice";
    const arcadeScore = Math.max(0, Math.min(999, Math.floor(Number(body.score || 0))));
    const roll = ["reactor", "memory", "signal"].includes(game) ? arcadeScore : crypto.randomInt(1, game === "dice" ? 7 : 101);
    const otReward = game === "spin"
      ? (roll > 96 ? 25 : roll > 84 ? 10 : 0)
      : (["reactor", "memory", "signal"].includes(game) && arcadeScore >= 160 ? Math.min(80, Math.floor(arcadeScore / 8)) : 0);
    const xp = ["reactor", "memory", "signal"].includes(game)
      ? Math.min(260, SOCIAL_XP_RULES.game + Math.floor(arcadeScore * 1.4))
      : SOCIAL_XP_RULES.game + (game === "dice" ? roll : Math.ceil(roll / 20));
    if (otReward) addOt(auth.user, otReward, "minigame", game);
    addXp(auth.user, xp, `minigame:${game}`);
    grantAchievement(auth.user, "game-streak");
    if (["reactor", "memory", "signal"].includes(game)) grantAchievement(auth.user, "arcade-rookie");
    if (arcadeScore >= 120) grantAchievement(auth.user, "reactor-master");
    const round = {
      id: uid("game"),
      userId: auth.user.id,
      game,
      result: { roll, score: arcadeScore, label: game === "coin" ? (roll % 2 ? "Орел" : "Решка") : (["reactor", "memory", "signal"].includes(game) ? `${arcadeScore} очков` : String(roll)) },
      xp,
      ot: otReward,
      createdAt: now()
    };
