...
const ADMIN_LOGIN_PASSWORD = String(process.env.ADMIN_LOGIN_PASSWORD || "123487");
...
function normalizeStore(parsed) {
  ...
  store.schemaVersion = Math.max(Number(store.schemaVersion || 0), 7);
  store.users ||= [];
  store.sessions ||= [];
  store.messages ||= [];
  store.groups ||= [];
