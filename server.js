...
function deleteAccountByAdmin(target, adminUser, reason = "") {
  ...
  for (const user of db.users) {
    ...
    user.favoriteRoomIds = (user.favoriteRoomIds || []).filter((roomId) => !dmParticipants(roomId).includes(targetId) && supportUserIdFromRoom(roomId) !== targetId);
    user.mutedRoomIds = (user.mutedRoomIds || []).filter((roomId) => !dmParticipants(roomId).includes(targetId) && supportUserIdFromRoom(roomId) !== targetId);
    user.savedMessageIds = (user.savedMessageIds || []).filter((id) => {
      const message = db.messages.find((item) => item.id === id);
      return message && message.senderId !== targetId && !dmParticipants(message.roomId).includes(targetId) && supportUserIdFromRoom(message.roomId) !== targetId;
    });
  }

  for (const message of db.messages) {
    if (dmParticipants(message.roomId).includes(targetId) || supportUserIdFromRoom(message.roomId) === targetId) {
...
function messageTargets(message) {
  ...
  return ids.filter((id) => id === message.senderId || isSupportRoom(message.roomId) || !isBlockedBetween(id, message.senderId));
}
...
function usersForRoom(roomId, senderId) {
  const targets = roomTargets(roomId);
  const ids = targets === "all" ? db.users.map((user) => user.id) : targets;
  return ids.filter((id) => id && id !== senderId && (isSupportRoom(roomId) || !isBlockedBetween(id, senderId)));
}
...
function serializeAdminMessage(message, adminUser) {
  ...
  data.sender = isSupportRoom(message.roomId) && sender && isAdmin(sender) ? supportAvatarUser() : (sender ? publicUserFor(sender, adminUser) : {
    id: message.senderId,
