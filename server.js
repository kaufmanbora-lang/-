...
const PAIR_GAME_TYPES = new Set(["duel", "memory-race", "signal-clash"]);
...
function serveStatic(req, res, pathname) {
  ...
  if (!STATIC_FILES.has(requested) && !isNftAsset) {
...
async function handleApi(req, res, url) {
  ...
  if (url.pathname === "/api/minigames/play" && req.method === "POST") {
    ...
    const arcadeScore = Math.max(0, Math.min(999, Math.floor(Number(body.score || 0))));
    const roll = arcadeScore;
    const otReward = game === "spin"
      ? (arcadeScore >= 260 ? 25 : arcadeScore >= 180 ? 10 : 0)
      : (arcadeScore >= 220 ? Math.min(80, Math.floor(arcadeScore / 8)) : 0);
    const xp = Math.min(280, SOCIAL_XP_RULES.game + Math.floor(arcadeScore * (game === "spin" ? 0.9 : 1.2)));
