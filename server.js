...
async function handleApi(req, res, url) {
  ...
  if (url.pathname === "/api/register/request-code" && req.method === "POST") {
    ...
    if (!mailConfigured() && requireRealRegistrationEmail()) {
      return sendError(res, 503, "Письмо с кодом не может быть отправлено: настрой RESEND_API_KEY или SMTP-переменные на Render.");
    }
