...
- Registration email codes now require real email delivery on Render; if SMTP/Resend is missing, the server returns a setup error instead of pretending the code was sent.
...
- Registration now requires accepting Terms and Privacy before email-code request and account creation; the acceptance timestamps are stored on the account.
