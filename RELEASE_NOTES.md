# Orbit Chat 1.5.9 - Logout and App Store Terms

## Account controls

- Added a visible `Выйти из аккаунта` button inside the profile panel.
- Wired the profile logout button to the existing logout flow with a confirmation prompt.
- Added profile links to the user agreement, privacy policy, and support page.

## App Store terms

- Rebuilt `terms.html` as a fuller user agreement for a messenger/social app.
- Covered accounts, user-generated content, reports, blocking, moderation, account deletion, calls/media, privacy, payments, and Apple App Store conditions.

## QA

- Added checks for the profile logout button, legal links, version `1.5.9`, and required App Store terms sections.

# Orbit Chat 1.5.8 - Mobile Controls and Security Menu

## Mobile layout fix

- Restored all phone chat tools: notes, export, polls, tasks, events, focus, compact mode, emoji, and templates are visible again.
- Rebuilt the mobile chat header into a 7-button grid instead of a cramped horizontal squeeze.
- Rebuilt the mobile side actions into a 5-button grid.
- Fixed the mobile composer grid so attach, voice, message input, and send stay in the correct order.
- Added a bottom-tab Security entry for fast phone access.

## Security menu

- Turned Security into a clear menu with session reset, profile/email, notifications, privacy, terms, rules, and account deletion.
- Kept account deletion isolated in a danger zone with confirmation.
...
- `node qa-check.js`
- Mobile layout static checks for restored feature buttons and Security tab.
...
- Registration now requires a 6-digit email code before the account is created.
