...
## July 4 3D dice, cleaner OT coin, and pair-game invite update

- Added a local 3D dice asset and local model-viewer bundle, with a CSS 3D fallback for phones or offline loading.
- Reduced and straightened the OT coin, then replaced the flip with a single toss-and-stop animation.
- Fixed OT Spin animation restarts: the wheel now performs one calculated spin and stays locked until the animation ends.
- Added extra online/local pair modes: D6 Bank, OT Coin Clash, and OT Spin Battle.
- Added quick friend invite chips in pair-game setup.

## July 4 OT Spin and token animation update

- Rebuilt the Dice visual into a layered 3D cube with front, side, and top faces plus a stronger roll animation.
- Reworked the OT coin toward the provided black-blue token reference: larger beveled coin, ridged edge, neon rings, non-mirrored OT logo, and cleaner flip motion.
- Made OT Spin paid: every spin costs 500 OT, the server checks balance, deducts the cost, and immediately grants the rolled OT reward.
- Added the requested OT Spin weighted reward table: 150, 250, 350, 450, 500, 550, 650, 750, 850, 950, 1000, 2000, and 2500 OT, with 2500 as the rarest result.

## July 4 games polish update

- Added obvious local two-player games on one device: Космо-дуэль, Парная память, and Колесо-батл.
- Kept online pair challenges compatible with the server: Космо-дуэль, Парная память, and Сигнал-гонка.
- Rebuilt Memory so it uses four big Simon-style colored pads instead of the Reactor grid.
- Rebuilt the OT coin as a glossy black-blue token with OT lettering, neon rings, and a flip animation.
- Rebuilt the wheel of fortune with neon sectors, pointer, center OT logo, stronger animation, and result label.
- Fixed Orbit+ panel scrolling so long game screens and pair-game cards are reachable on desktop and phone.
- Added QA markers for local pair games, OT coin visuals, the new memory board, and the new fortune wheel.
- Verified desktop UI, mobile 390x844 UI, client syntax, server syntax, QA, and Orbit+ smoke test.

## Grand games, video stories, and NFT gallery update

- Rebuilt the Orbit+ games screen so all six solo games open as different playable modes instead of repeated copies: Reactor, Memory, Signal, Dice, Coin, and OT Spin.
- Added unique game controls and visuals: active reactor cells, memory sequences, signal lanes, a large dice arena, coin guessing, and a spinning OT wheel.
- Added three two-player pair challenges: Space Duel, Memory Race, and Signal Clash. Both players submit a score, the server compares results, awards XP, and unlocks pair-game achievements.
- Added server APIs for pair-game create/play/cancel plus live Orbit+ updates for both players.
- Added video stories: story uploads now accept image or video media, and the client renders videos with native controls.
- Added account NFT galleries so public/profile data can show all owned NFTs, not only the one equipped NFT.
- Expanded automated checks and smoke tests to cover video stories, pair-game completion, pair-game achievements, and new Orbit+ UI markers.
- Verified every solo game in the in-app browser: each mode opens its own arena and score changes after its main action.
- Verified the mobile 390px games layout has no horizontal overflow and still exposes all six games plus pair-game creation.
