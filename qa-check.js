...
function checkClientScript() {
  ...
  for (const action of ["chats", "calls", "contacts", "favorites", "profile"]) {
  ...
  if (!html.includes("Письмо не отправлено, потому что почта ещё не настроена")) throw new Error("Registration UI must not pretend email was sent when mail is not configured.");
...
function checkPackageAndRenderConfig() {
  ...
  if (!server.includes("requireRealRegistrationEmail")) throw new Error("server.js must require real registration email delivery in production.");
