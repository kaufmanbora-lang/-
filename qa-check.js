...
function checkPackageAndRenderConfig() {
  ...
  for (const api of ["/api/orbit-plus", "/api/stories", "/api/stories/react", "/api/friends/request", "/api/friends/action", "/api/customization/buy", "/api/customization/equip", "/api/minigames/play", "/api/clans", "/api/preferences"]) {
    if (!server.includes(api)) throw new Error(`server.js must expose Orbit+ API: ${api}.`);
  }
  for (const stateKey of ["stories", "friendRequests", "clans", "miniGameRounds", "friendIds", "achievementIds", "customInventory", "notificationPrefs"]) {
