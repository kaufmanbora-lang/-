...
function checkClientScript() {
  ...
  if (!html.includes('id="profileLogoutButton"')) throw new Error("Profile must include a visible logout button.");
  if (!html.includes("profile-doc-links")) throw new Error("Profile must include accessible legal/support links.");
  if (!html.includes('id="registerCode"')) throw new Error("Registration must include email code input.");
  if (!html.includes('id="requestRegisterCodeButton"')) throw new Error("Registration must include request-code button.");
  if (!html.includes('id="profileEmailCode"')) throw new Error("Profile must include email code confirmation input.");
  if (!html.includes('id="confirmEmailCodeButton"')) throw new Error("Profile must include confirm email code button.");
  ...
  if (!html.includes("Orbit Chat 1.5.8: phone control rescue")) throw new Error("index.html must include the 1.5.8 mobile control rescue layer.");
  if (!html.includes("Orbit Chat 1.5.9: visible profile logout")) throw new Error("index.html must include the 1.5.9 logout/App Store terms layer.");
  ...
  for (const action of ["chats", "calls", "contacts", "favorites", "security", "profile"]) {
  ...
  if (html.includes(".chat-tools #roomNoteButton,\n      .chat-tools #exportChatButton,\n      .chat-tools #pollsButton {\n        display: none;")) {
    throw new Error("Mobile layout must not hide room note, export, or polls buttons.");
  }
...
function checkPackageAndRenderConfig() {
  ...
  if (pkg.version !== "1.5.9") throw new Error("package.json version must be 1.5.9.");
  ...
  if (!server.includes("/api/register/request-code")) throw new Error("server.js must include registration email-code request endpoint.");
  if (!server.includes("/api/email/confirm-code")) throw new Error("server.js must include email-code confirmation endpoint.");
  if (!server.includes("emailCodes")) throw new Error("server.js must persist one-time email codes.");
  if (!server.includes("sendEmailCode")) throw new Error("server.js must send one-time email codes.");
