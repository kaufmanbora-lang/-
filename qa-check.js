...
function checkManifest() {
  ...
  if (!Array.isArray(manifest.icons) || manifest.icons.length < 3) throw new Error("manifest.icons must include regular and maskable PNG icons.");
...
function checkAndroidApkPackage() {
  const packageRoot = path.join(root, "android-apk-package");
  ...
  const requiredFiles = [
    ...
    "app/eas.json",
    "app/package.json",
    "app/BUILD_ANDROID_APK.cmd",
    "app/CHECK_ANDROID.cmd",
    "app/SHOW_LATEST_BUILDS.cmd",
    "app/scripts/setup-android.js",
    "app/scripts/preflight-android.js",
  ...
  for (const file of requiredFiles) {
    if (!fs.existsSync(path.join(packageRoot, file))) {
      throw new Error(`Android APK package is missing ${file}.`);
    }
  }

  runNodeCheck(path.relative(root, path.join(appRoot, "scripts", "setup-android.js")));
  runNodeCheck(path.relative(root, path.join(appRoot, "scripts", "preflight-android.js")));
  ...
  const buildScript = fs.readFileSync(path.join(appRoot, "BUILD_ANDROID_APK.cmd"), "utf8");

  if (appJson.expo?.android?.allowBackup !== false) throw new Error("Android backup must be disabled.");
  if (!appJson.expo?.android?.permissions?.includes("android.permission.CAMERA")) throw new Error("Android camera permission is missing.");
  if (!appJson.expo?.android?.permissions?.includes("android.permission.RECORD_AUDIO")) throw new Error("Android microphone permission is missing.");
  if (easJson.build?.apk?.android?.buildType !== "apk") throw new Error("EAS must produce an installable APK.");
  if (easJson.build?.apk?.distribution !== "internal") throw new Error("EAS APK must use internal distribution.");
  for (const marker of [
    "OrbitChat-Android/1.6",
    "orbit-native-android-app",
    "hardwareBackPress",
    'mixedContentMode="never"',
    "onRenderProcessGone"
  ]) {
    if (!appSource.includes(marker)) throw new Error(`Android wrapper marker is missing: ${marker}.`);
  }
  if (!buildScript.includes("npm run build:apk")) throw new Error("Android one-click build launcher is missing.");
  if (!packageJson.scripts?.["build:apk"]?.includes("eas-cli@21.3.0 build")) {
    throw new Error("Android APK build command is missing.");
  }
}
...
function checkClientScript() {
  ...
  if (!html.includes('id="startupVideo"')) throw new Error("Startup splash must include the launch video.");
  if (!html.includes('addEventListener("ended", closeStartupSplash')) throw new Error("Startup splash must close after the launch video ends.");
  ...
  if (!html.includes("function nftSupplyHtml(") || !html.includes("Тираж распродан")) throw new Error("NFT market must display limited supply and sold-out state.");
  ...
  for (const marker of ["applyCustomizationStyles", "profileBgClass", "messageEffectClass", "shopPreviewHtml", "style-preview", "story-open-link"]) {
    if (!html.includes(marker)) throw new Error(`Orbit+ customization/story video UI missing: ${marker}.`);
  }
...
function checkPackageAndRenderConfig() {
  ...
  if (fs.existsSync(path.join(root, "app-store-package"))) {
    for (const file of [
      ...
      "app-store-package/ios-wrapper/store.config.js",
      "app-store-package/ios-wrapper/package-lock.json",
      "app-store-package/ios-wrapper/START_APP_STORE.cmd",
      "app-store-package/ios-wrapper/scripts/setup-release.js",
      "app-store-package/ios-wrapper/scripts/preflight.js",
  ...
  for (const file of ["assets/brand/orbit-chat-logo.jpg", "assets/brand/orbit-chat-launch.mp4"]) {
    if (!fs.existsSync(path.join(root, file))) throw new Error(`${file} is missing.`);
  }
  if (!htmlSource.includes('id="startupVideo"') || !htmlSource.includes("/assets/brand/orbit-chat-launch.mp4")) {
    throw new Error("index.html must use the branded launch video.");
  }
  if (!htmlSource.includes("/assets/brand/orbit-chat-logo.jpg")) throw new Error("index.html must use the new Orbit Chat logo.");
  if (!server.includes('"/assets/brand/orbit-chat-launch.mp4"')) throw new Error("server.js must serve the launch video from the static allowlist.");
  if (!server.includes('".mp4": "video/mp4"') || !server.includes('"accept-ranges": "bytes"')) {
    throw new Error("server.js must support streamed MP4 playback on mobile browsers.");
  }
  if (!serviceWorkerSource.includes('request.headers.has("range")')) {
    throw new Error("sw.js must pass video range requests through to the server.");
  }
  ...
  if (server.includes('["123", "487"]')) throw new Error("server.js must not contain the legacy admin password.");
  if (server.includes("db.users[0]?.id === user.id")) throw new Error("The first user must never receive implicit admin rights.");
  if (!server.includes('const PASSWORD_ALGORITHM = "scrypt-v1"')) throw new Error("New passwords must use scrypt.");
  if (!server.includes("sessionTokenHash") || !server.includes("tokenHash: sessionTokenHash(token)")) throw new Error("Sessions must be persisted as token hashes.");
  if (!server.includes("requireCsrf") || !server.includes("x-orbit-csrf")) throw new Error("Authenticated mutations must require CSRF protection.");
  if (!htmlSource.includes('headers["x-orbit-csrf"]')) throw new Error("Client must send the CSRF token.");
  ...
  if (!server.includes("isNativeIosAppRequest")) throw new Error("server.js must identify the native iOS wrapper.");
  if (!server.includes('url.pathname === "/api/ot/purchase-request"') || !server.includes("if (isNativeIosAppRequest(req))")) {
    throw new Error("server.js must disable external OT purchase requests on iOS.");
  }
  if (!htmlSource.includes("orbit-native-ios-app")) throw new Error("index.html must include native iOS layout rules.");
  ...
  if (!server.includes("schemaVersion: 9")) throw new Error("server.js schemaVersion must be 9.");
  ...
  if (!server.includes("NFT_CHANEL_BACKGROUNDS") || !server.includes("nft-chanel-sprite.jpg") || !server.includes('symbol.id === "chanel"')) throw new Error("server.js must include the Chanel NFT catalog.");
  if (!server.includes('symbol.id === "lokochka"') || !server.includes("sold * 650")) throw new Error("Lokochka NFT must use uncapped price growth.");
  if (!server.includes('"chanel-neon": 10000') || !server.includes('"chanel-royal": 25000') || !server.includes('"chanel-dream": 21000') || !server.includes("sold * 450")) throw new Error("Chanel NFT must make background 4 the most expensive base variant with uncapped growth.");
  if (!server.includes("NFT_ROLEX_BACKGROUNDS") || !server.includes("nft-rolex-sprite.jpg") || !server.includes('symbol.id === "rolex"')) throw new Error("server.js must include the Rolex NFT catalog.");
  if (!server.includes('"rolex-blue": 9999') || !server.includes('"rolex-violet": 15000') || !server.includes('"rolex-obsidian": 14700') || !server.includes('"rolex-heart": 13800') || !server.includes("sold * 300")) throw new Error("Rolex NFT must make background 2 the most expensive base variant with uncapped growth.");
  if (!server.includes("NFT_SPIKE_BACKGROUNDS") || !server.includes("nft-spike-sprite.jpg") || !server.includes('symbol.id === "spike"')) throw new Error("server.js must include the Spike NFT catalog.");
  if (!server.includes('"spike-blue": 5555') || !server.includes('"spike-cosmos": 7777') || !server.includes('"spike-city": 6999') || !server.includes("sold * 222")) throw new Error("Spike NFT must make background 2 the most expensive base variant with uncapped growth.");
  if (!server.includes("NFT_KAYLI_BACKGROUNDS") || !server.includes("nft-kayli-sprite.jpg") || !server.includes('symbol.id === "kayli"')) throw new Error("server.js must include the Kayli NFT catalog.");
  if (!server.includes('"kayli-home": 10000') || !server.includes('"kayli-moon": 20000') || !server.includes("sold * 360")) throw new Error("Kayli NFT must use 10 000-20 000 OT base prices with the standalone image as the most expensive variant.");
  for (const marker of ["lokochka: 1000", "chanel: 5000", "rolex: 10000", "spike: 15000", "kayli: 20000"]) {
    if (!server.includes(marker)) throw new Error(`server.js missing fixed NFT edition limit: ${marker}.`);
  }
  if (!server.includes("function nftEditionLimit(") || !server.includes("soldBefore >= editionLimit")) throw new Error("Primary NFT market must enforce limited editions before charging OT.");
  if (server.includes('name: "Chanel') || server.includes('name: "Rolex') || server.includes('name: "Spike')) throw new Error("Special NFT backgrounds must keep generic names like Фон 1, Фон 2.");
  if (server.includes("Math.min(15000 - basePrice") || server.includes("Math.min(25000 - basePrice") || server.includes("Math.min(55000 - basePrice")) throw new Error("Special NFT prices must not be capped by the base-price range.");
  if (server.includes("return symbol.variants?.[background.id] || symbol.image") || htmlSource.includes("return symbol?.variants?.[background?.id] || symbol?.image")) throw new Error("NFT variants must not fall back to unrelated backgrounds.");
  ...
  for (const marker of ["nicknameColor", "profileBackground", "nameColor", "profileBg", "canSeeStory(user, story)", "type.startsWith(\"video/\")"]) {
    if (!server.includes(marker)) throw new Error(`server.js missing customization/story-video marker: ${marker}.`);
  }
